---
title: manjaro的安装与配置
date: 2021-06-28 18:14:11
tags:
    - manjaro
    - vim
categories: linux
---
由于经常用到linux系统，相比之下manjaro深得人心，避免以后换电脑又到处折腾，在这里记录一下自己的配置和走过的弯路。
<!--more-->
![](neofetch.png)
## manjaro安装
```bash
...
写入重启就能输入中文了
## zsh和ohmyzsh
manjaro自带zsh，omz因为某些原因下载不了，我们要进入以下网址：https://www.ipaddress.com/ 输入githubusercontent.com查询ip，修改host
```bash
sudo vim /etc/hosts
```
在里面加入查询到的ip
...
等待编译完成,可能会遇到的错误
```
export GOPROXY=http://goproxy.io("go 模块设置代理")
sudo pacman -S openjdk-11-jdk （安装java）
```
最后copy以下vim配置
```bash
set nocompatible
...
```
等安装完成，失败多试几次
我加了fortran语法，F2打开目录，F3显示函数，zc自动折叠函数
![](vim.png)
## clash 及开机自启动
下载clash
```
...
```
运行clash，生成配置文件到
~/config/clash/,包含config.yaml和country.mmdb
``````
#开机启动
